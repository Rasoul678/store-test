...
<nav class="navbar navbar-expand-sm navbar-dark bg-dark pt-0 pb-1">
      <div class="container m-0">
            ...
            <div class="collapse navbar-collapse" id="main_nav">
                  <ul class="navbar-nav">
                        <li class="nav-item">
                              <a class="nav-link text-warning p-0" href="{{ url('/') }}" style="font-size: 22px">Home</a>
                        </li>
                        @foreach($categories as $cat)
                              @foreach($cat->items as $category)
                                    @if ($category->items->count() > 0)
                                          <li class="nav-item dropdown">
                                                <a class="nav-link dropdown-toggle text-light ml-3 p-0 mt-1" href="{{ route('category.show', $category->slug) }}" id="{{ $category->slug }}"
                                    ...
                                    @else
                                          <li class="nav-item">
                                                <a class="nav-link text-light ml-3 p-0 mt-1" href="{{ route('category.show', $category->slug) }}" style="font-size: 18px">{{ $category->name }}</a>
                                          </li>
                                    @endif
                              @endforeach
                        @endforeach
